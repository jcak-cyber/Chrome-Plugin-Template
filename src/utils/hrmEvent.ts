...
// 创建热更新管理器
export function createHMRManager(render: () => void): HMRManager {
  ...
  function handleModuleUpdate(msg: any) {
    ...
    // 对于vue文件，尝试进行vue组件热更新
    else if (msg.extension === ".vue") {
      updateVueModule(msg);
    } else {
      // 对于其他文件，刷新窗口
      window.location.reload();
  ...
  // 更新VUE模块
  function updateVueModule(msg: any) {
    console.log("Vue component changed, attempting hot update...");
    ...
    try {
      ...
      if (renderCallback) {
        renderCallback();
        console.log("Vue component hot-updated successfully");
      ...
      console.error("Failed to hot-update Vue component:", error);
      // 如果热更新失败，回退到完全刷新
      window.location.reload();
    }
  }
